...
class ConditionalEntropyLoss(torch.nn.Module):
    def __init__(self, dataset_type="icu", task="decompensation"):
    ...
    def forward(self, x):
        #for binary classification, we only return probs (not unnormalized scores)
        if self.dataset_type == "icu" and self.task != "los":
            b = x * torch.log(x+self.eps) + (1-x) * torch.log(1-x+self.eps)
            b = b.sum(dim=1)
...
class LMMD_loss(nn.Module):
    ...
    def get_loss(self, source, target, s_label, t_label):
        ...
        loss += torch.sum(weight_ss * SS)
        loss += torch.sum(weight_tt * TT)
        loss += torch.sum(-2 * weight_st * ST)
    ...
    def cal_weight(self, s_label, t_label, batch_size=32, class_num=4):
        ...
        s_sca_label = s_label.cpu().data.numpy().flatten()
        #YO addition one line
        s_sca_label = s_sca_label.astype(int)
