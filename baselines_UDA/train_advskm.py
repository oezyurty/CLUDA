...
from models.models import ReverseLayerF, AdvSKM_Disc
...
def main(args):
    ...
    AdvSKM_embedder = AdvSKM_Disc(input_dim = 64, hidden_dim = 256)
    ...
    classifier.cuda()
    AdvSKM_embedder.cuda()

    mmd_loss_calc = MMD_loss(kernel_num=1)
    ...
    for i in range(args.num_epochs):
        ...
        for i_batch, sample_batched_src in enumerate(dataloader_src):
            ...
            mmd_loss = - mmd_loss_calc(source_embedding_disc, target_embedding_disc)
            ...
            mmd_loss_adv = mmd_loss_calc(source_embedding_disc, target_embedding_disc)
            mmd_loss_adv.requires_grad = True

            loss =  args.weight_domain*mmd_loss_adv + src_cls_loss
            ...
            # measure domain loss
            losses_ts.update(mmd_loss_adv.item(), sample_batched_src['sequence'].size(0))
            ...
            if count_step % args.checkpoint_freq == 0:
                ...
                for i_batch_val in range(args.num_val_iteration):
                    ...
                    # measure domainloss
                    losses_ts.update(mmd_loss_adv.item(), sample_batched_val_src['sequence'].size(0))
                ...
                if cur_val_score > best_val_score:
                    ...
                    torch.save({'feature_extractor_state_dict': feature_extractor.state_dict(),
                        'AdvSKM_embedder_state_dict': AdvSKM_embedder.state_dict(),
