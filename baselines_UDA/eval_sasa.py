...
def main(args):
    ...
    feature_extractor = SASA(x_dim=dataset_test_src[0]['sequence'].shape[1], h_dim=saved_args.h_dim, n_segments=saved_args.n_segments, seg_length=saved_args.seg_length)


    classifier = MLP(input_dim = saved_args.h_dim * 2 * dataset_test_src[0]['sequence'].shape[1]  + dataset_test_src[0]['static'].shape[0], hidden_dim = 256, 
    ...
    for i_batch, sample_batched in enumerate(dataloader_test_trg):
        ...
        _, _, trg_feat = feature_extractor(sample_batched['sequence'])
    ...
    for i_batch, sample_batched in enumerate(dataloader_test_src):
        ...
        _, _, src_feat = feature_extractor(sample_batched['sequence'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-emf', '--experiments_main_folder', type=str, default='experiments_AdvSKM')
